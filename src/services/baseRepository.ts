...
export class baseRepository {
	...
	// トレイニー文書の取得または作成
	static async getTraineeDocument(): Promise<void> {
		const auth = getAuth(firebaseApp);
	...
	// トレイニー情報の取得
	static async getTraineeData() {
		await baseRepository.getTraineeDocument();
		const auth = getAuth(firebaseApp);
		const user = auth.currentUser;
	...
	// トレイニー体重の更新
	static async updateTraineeWeight(weight: number) {
		const auth = getAuth(firebaseApp);
		const user = auth.currentUser;
		if (!user) throw new Error('認証ユーザーが見つかりません');
		const db = getFirestore(firebaseApp);
	...
	// トレーニングプランの作成
	static async createTrainingPlan(trainingPlan: TrainingPlan): Promise<void> {
		const db = getFirestore(firebaseApp);
		try {
